...
class ProveedorController extends AdminBaseController
{
    ...
    {
        return view('proveedores::admin.proveedors.index');
    }
    ...
    public function search_proveedor( Request $request )
    {
        ...
        if ($request->has('term')  && trim($request->has('term') !== '') )
        {
            $queries = Proveedor::where('razon_social', 'like', "%{$request->get('term')}%")
                            ->orWhere('ruc','like',"%{$request->get('term')}%")
                            ->take(7)
        ...
        {
            $results[] = [ 'id' => $query->id, 'value' => $query->razon_social . ". RUC: " .$query->ruc, 'ruc' => $query->ruc];
        }
    ...
    public function create()
    {
        ...
        return view('proveedores::admin.proveedors.create', compact('proveedor'));
    }
    ...
    public function destroy(Proveedor $proveedor)
    {
        ...
        catch (\Illuminate\Database\QueryException $e)
        {
            if($e->getCode() == "23000")
            {
                flash()->error('No se puede eliminar. El proveedor está asociado alguna compra. ');
