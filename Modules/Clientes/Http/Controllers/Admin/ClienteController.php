...
class ClienteController extends AdminBaseController
{
    ...
    public function create()
    {
        $cliente = new Cliente;
        $cliente->activo = true;
        return view('clientes::admin.clientes.create', compact('cliente') );
    }
    ...
    public function store(ClienteRequest $request)
    {
        ...
        {
            if( $request['isVentaCliente'] or $request['isPreVentaCliente'] )
                if(isset($cliente))
    ...
    public function destroy(Cliente $cliente)
    {
        ...
        catch (\Illuminate\Database\QueryException $e)
        {
            if($e->getCode() == "23000")
            {
              flash()->error('No se puede eliminar. El cliente está asociado a alguna venta.');
              return redirect()->back();
            }
            else
            {
