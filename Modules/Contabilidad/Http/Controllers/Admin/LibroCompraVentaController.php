...
class LibroCompraVentaController extends AdminBaseController
{
    ...
    public function libro_venta_excel(Request $re)
    {
        ...
        {
            $gravado_excenta = ($registro->gravado_excenta>0)?$registro->gravado_excenta:0;

            $gravado_5 = ($registro->gravado_5>0)?$registro->gravado_5:0;

            $gravado_10 = ($registro->gravado_10>0)?$registro->gravado_10:0;

            $total_iva_5 = ($registro->total_iva_5>0)?$registro->total_iva_5:0;

            $total_iva_10 = ($registro->total_iva_10>0)?$registro->total_iva_10:0;

            $total_gravado = ($registro->total_gravado > 0)?$registro->total_gravado:0;
            ...
            [
                ...
                'Subtotal Exento' => $gravado_excenta, 
                'Subtotal Gravado 5%' => $gravado_5, 
                'Subtotal Gravado 10%' => $gravado_10,
                'IVA 5%' => $total_iva_5,
                'IVA 10%' => $total_iva_10,
                'Total' => $total_gravado,
                'Fecha de Doc: ' . $fecha => '',
     ...
     public function libro_compra_excel(Request $re)
    {
        ...
        foreach ($libro_compras as $key => $registro)
        {
            ...
            $total_gravado_iva = ($registro->total_gravado_iva > 0)?$registro->total_gravado_iva:0;
