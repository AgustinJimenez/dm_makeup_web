...
<?php /*----------------------------------------------------------------------------------------------------------------------------*/
    $columnTipo = '<td class="col-sm-1" >
            ...
            if( \Config::$productos ):
            $columnTipo .= '<option value="producto" checked>Producto</option>';
            endif;
            if( \Config::$servicios ):
            $columnTipo .=  '<option value="servicio" >Servicio</option>';
            endif;
            $columnTipo .=  '</select>
                    </td>';
    if (!count($errors))
...
if( \Config::$productos ):
    $columnSeleccion .= '<div id="divProductos" class="form-group" >
...
if( \Config::$servicios ):
    $div_servicios_displayed = ( \Config::$productos )?'style="display:none;"':'';
    $div_servicios_required = ( \Config::$productos )?'':'required="required"';
    $columnSeleccion .= '<div id="divServicios" ' . $div_servicios_displayed . ' class="form-group">
                            <input type="text" class="form-control input-sm servicio" name="nombre_servicio[]" id="nombre_servicio" value="" placeholder="Nombre Servicio" ' . $div_servicios_required . '>
                        </div>';
endif;

$columnSeleccion .= '<div class="seleccionID" id="seleccionID" style="display:none" margin-top: 0%; float:left;">
...
$columnSeleccion = '<td class="col-md-2 input-group">';
    ...
    $columnSeleccion .= '<div id="divServicios" style="display:none;" class="form-group">
...
$columnSeleccion .= '<div class="seleccionID" id="seleccionID" style="display:none" margin-top: 0%; float:left;">';
    if( \Config::$productos ):
    $columnSeleccion .= '<input type="text" class="form-control input-sm  seleccion" name="producto_id[]" id="producto_id" value="" placeholder="" readonly style="height:20px;width:40px;'.$debug.'">';
    endif;
    if( \Config::$servicios ):
    $columnSeleccion .= '<input type="text" class="form-control input-sm  seleccion" name="servicio_id[]" id="servicio_id" value="" placeholder="" readonly style="height:20px;width:40px;'.$debug.'">';
    endif;
    $columnSeleccion .= '<input type="text" class="form-control input-sm  seleccion" name="cristal_id[]" id="cristal_id" value="" placeholder=" readonly style="height:20px;width:40px;'.$debug.'">
                    </div>
                </td>';
