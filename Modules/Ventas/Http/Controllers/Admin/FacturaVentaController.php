...
class FacturaVentaController extends AdminBaseController
{
    ...
    public function index(Request $re)
    {
        ...
        {
            $isOtros = true;
            return view('ventas::admin.facturaventas.index', compact('isOtros','hoy','desde'));
        }
        else
            return view('ventas::admin.facturaventas.index',compact('hoy','desde'));
    }
    ...
    public function index_ajax(Request $request)
    {
        ...
        if ($request->has($fecha_inicio) && trim($request->has($fecha_inicio) !== '') )
            $query->where('fecha_venta', '>', date_to_server2($request[$fecha_inicio])->modify('-1 day')  );

        if ($request->has($fecha_fin) && trim($request->has($fecha_fin) !== '') )
            $query->where('fecha_venta', '<', date_to_server2($request[$fecha_fin])  );
...
*/
        ...
        else
        {
            ...
            else
            {
                ...
                return view('ventas::admin.facturaventas.edit', compact('facturaventa','detalles', 'pendiente','autorizar','venta','hay_preventa'));
            }
        }
