...
class VentaController extends AdminBaseController
{
    ...
    public function index_preventa()
    {
        if(!\Config::$preventa )
        {
            flash()->error('La configuracion del sistema no permite las preventas');
    ...
    public function create(Request $request )
    {
        ...
        $visitas_list[''] = "Ninguna";
        $visitas_list_tmp = \Visita::select("user_id", "lugar","id")->where("user_id", $request->user()->id)->where("estado", "abierta")->lists('lugar', 'id')->toArray();
        foreach ($visitas_list_tmp as $key => $value)
            $visitas_list[$key] = $value;
        return view('ventas::admin.ventas.create', compact('isVenta','isPreventa','isOtros','nro_factura','nro_seguimiento', 'fecha', 'haber',
            'debe_contado', 'debe_credito', "visitas_list","hay_preventa" ));
    ...
    public function store(Request $re)
    {
        ...
        for ($i=0; $i < count($re['cantidad']) ; $i++)
        {
            ...
            {
                $gravado_excenta += $precio_unitario*$cantidad;
            }
        }

        //descuento_total y descuento apliacado al iva
        $factor_de_descuento = $re['descuento_total'] / ($re['monto_total'] + $re['descuento_total']);
        $descuento_porcentaje = $factor_de_descuento*100;


        $gravado_excenta = intval($gravado_excenta*(1-$factor_de_descuento));
        $gravado_5 = intval($gravado_5*(1-$factor_de_descuento));
        $gravado_10 = intval($gravado_10*(1-$factor_de_descuento));
        $total_iva_10 = intval($total_iva_10*(1-$factor_de_descuento));
        $total_iva_5 = intval($total_iva_5*(1-$factor_de_descuento));


        $gravado_10 = $total_item_con_iva_10 - $total_iva_10;

        $gravado_5 = $total_item_con_iva_5 - $total_iva_5;

        $total_suma = $gravado_excenta + $gravado_5 + $gravado_10 + $total_iva_10 + $total_iva_5;

        //dd('EXCENTA='.$gravado_excenta.' GRAVADO 5% ='.$gravado_5.' GRAVADO 10% ='.$gravado_10.' IVA 5%='.$total_iva_5.' IVA 10%='.$total_iva_10.' TOTAL='.$re->monto_total.' TOTAL DE SUMA='.$total_suma);
...
*/
        ...
        try
        {
            ...
            ([
                ...
                'anulado' => false,
                'visita_id' => ($re['visita_id'] == '')?null:$re['visita_id']
            ]);
            ...
            else if($re['total_pagado'] > 0  && $tipo == 'preventa')
            {
                ...
                ([
                    'fecha' => $re['fecha_venta'],
                    'observacion' => 'Preventa  |  Cliente: '.$cliente,
                    'operacion' => 'Preventa',
...
/************************ASIENTOS*************************/
                if($re['tipo_factura'] == 'contado')
                {
                    ...
                    if ($tipo != 'otros')
                    {
                        ...
                        ([
                            'fecha' => $re['fecha_venta'],
                            'observacion' => 'Venta al Contado  | Cliente:  '.$venta->cliente->razon_social.' | Factura Nro:  '.$re['nro_factura'],
                    ...
                    else
                    {
                        ...
                        ([
                            'fecha' => $re['fecha_venta'],
                            'observacion' => 'Otras Ventas al Contado  | Cliente:  '.$venta->cliente->razon_social.' | Factura Nro:  '.$re['nro_factura'],
                            'operacion' => 'Venta Contado',
                            'usuario_create_id' => $this->auth->id(),
                            'entidad_type' => FacturaVenta::class,
                            'entidad_id' => $factura->id
                ...
                else if($re['tipo_factura'] == 'credito')
                {
                    if ($tipo != "otros")
                    {
                        ...
                        ([
                            'fecha' => $re['fecha_venta'],
                            'observacion' => 'Venta a Crédito | Cliente : '.$venta->cliente->razon_social.'  | Factura Nro: '.$re['nro_factura'],
                    ...
                    else
                    {
                        ...
                        ([
                            'fecha' => $re['fecha_venta'],
                            'observacion' => 'Otras Ventas a Crédito | Cliente : '.$venta->cliente->razon_social.'  | Factura Nro: '.$re['nro_factura'],
                            'operacion' => 'Venta a Crédito',
                            'usuario_create_id' => $this->auth->id(),
                            'entidad_type' => FacturaVenta::class,
                            'entidad_id' => $factura->id
...
*/
    ...
    public function edit(Venta $venta, Request $re)
    {
        ...
        $selections = array();
        if( \Config::$productos )
            $selections["producto"] = "Producto";
        if( \Config::$servicios )
            $selections["servicio"] = "Servicio";

        return view('ventas::admin.ventas.edit', compact('venta','isVenta','detalles','nro_factura', 'isOtros', 'permisos', 'selections'));
        ...
        try
        {
            ...
            else
            {
                ...
                if($request['generar_factura'])
                {
                    ...
                    ([
                        'fecha' => date('Y-m-d'),
                        'observacion' => 'Preventa  |  Cliente: '.$venta->cliente->razon_social.'  | Factura Nro: '.$venta->factura->nro_factura,
                        'operacion' => 'Ventas',
                        'usuario_create_id' => $this->auth->id(),
                        'entidad_type' => FacturaVenta::class,
                        'entidad_id' => $factura->id
                ...
                for($i = 0; $i < count($cantidad); $i++)
                {
                    ...
                    else
                    {
                        ...
                        if( $detalle_venta_id[$i]!='' )
                        {
                            ...
                            if($producto_id[$i]!=null && $servicio_id[$i]==null )
                            {
                                $stock_producto = DB::table('productos__productos')
                                    ->where('id',$producto_id[$i])->select('stock')
                                    ->first()->stock;
                        ...
                        else
                        {
                            ...
                            else if($producto_id[$i] && !$servicio_id[$i]  && $detalle_venta_id[$i]=='')
                            {
                                $stock_producto = DB::table('productos__productos')
                                    ->where('id',$producto_id[$i])
                                    ->select('stock')
