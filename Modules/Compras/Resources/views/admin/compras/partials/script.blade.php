...
*/
            ...
            $(window).keydown(function(event)
            {
                if(event.keyCode == 13)
                {
                    if( event.target.id == 'descripcion' && '{{ $isProducto }}'=="1" )
                    {
            ...
            function enter_was_presed_on_descripcion( element )
            {
                ...
                element.closest("tr").find(".cantidad").focus();
            }
                    ...
                    (
                        ...
                        function( data )
                        {
                            ...
                            if( response.status == "OK" ){
                                controlar_productos_repetidos(response.id, response.value);
            ...
            if(isProducto == '1')
            {
                ...
                $( ".descripcion" ).autocomplete(
                {
                    ...
                    select: function(event, ui)
                    {
                        controlar_productos_repetidos(ui.item.id,ui.item.value);
            ...
            function calculate_all()
            {
                ...
                $(".cantidad").each(function()
                {
                    ...
                    cantidad_total = parseInt(cantidad_total) +  parseInt(cantidad);
                    ...
                    $(this).closest('td').closest('tr').find('.numeracion').text( contador);
                ...
                $("#total_pagado").val($("#total_pagar").val());
                ...
                $("#cantidad_total_label").text("Cantidad (" + cantidad_total +")");

                $("#descripcion_label").text("Descripción (" + contador +")");
            ...
            function controlar_productos_repetidos( id, producto ){
              contador = 0;

              $(".seleccion").each(function(){
                contador += 1;
                if($(this).val() == id){
                  mensaje = 'El producto "' + producto +'" ya se encuentra en la lista. (fila ' + contador + '). De todas formas el producto ha sido agregado.';
                  mostrar_modal_alerta( mensaje);
                }
