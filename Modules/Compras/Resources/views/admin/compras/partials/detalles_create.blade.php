<div class="overflow-x:auto;" id="div_tabla">
    ...
    <table id="tabla_detalle_venta" class="table table-bordered table-striped table-highlight table-fixed tabla_detalle_venta">
        ...
        <tr>
            <th>#</th>
            <th class="col-sm-4 text-center" id="descripcion_label">Descripcion</th>
            <th class="col-sm-1 text-center" id="cantidad_total_label">Cantidad</th>
                    ...
                    @foreach(old('cantidad') as $key => $val)
                        @if($isProducto)
                            <td class="col-sm-4 input-group {!! $errors->first('producto_id.'.$key, 'has-error') !!}"  >
                                ...
                                <input type="text" class="form-control input-sm descripcion" name="descripcion[]" id="descripcion" value="{{ old('descripcion.'.$key) }}" placeholder="Nombre Producto" required="">
                        ...
                        @else
                            ...
                            <td class="col-sm-4 input-group">
                                ...
                                <input type="text" class="form-control input-sm descripcion @if($isProducto) search-producto-by-codigo @endif" name="descripcion[]" id="descripcion" value="{{ old('descripcion.'.$key) }}" placeholder="Descripcion" required = "" style="display: none">
...
<?php
          $row = '<td class="numeracion"></td>';
        if($isProducto)
        {
            $row =   $row.'<td class="col-sm-4 input-group"  >

                        <input type="text" class="form-control input-sm descripcion search-producto-by-codigo" name="descripcion[]" id="descripcion" value="" placeholder="Nombre Produto" required="">
    ...
    if ($isOtro){
      $row = $row. '<td class="col-sm-1" style="display:noe">
                          <select class="form-control iva" name="iva[]" id="iva" >
