...
class CompraController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        return view('compras::admin.compras.index',compact('hoy','desde'));
    }
    ...
    public function create(Request $request)
    {
        ...
        $tipos_comprobantes = Compra::getTiposComporbantes();

        $tipos_comprobantes_sin_factura = Compra::getTiposComporbantesSinFactura();

        $fecha_hoy = date('d/m/Y', time());

        return view('compras::admin.compras.create', compact('comprasProductos','isProducto','isServicio','isCristal','isOtro','fecha_hoy', 'nro_factura','haber', 'debe', 'haber_contado', 'haber_credito','tipos_comprobantes','tipos_comprobantes_sin_factura'));
    }
    ...
    public function store(CompraRequest $request, Authentication $usuario)
    {
        ...
        */
        ...
        $comprobante = true;//$request->get('comprobante');
...
/*----------------------------------------------------------------------------*/
        ...
        {
            $precio_unitario = intval( str_replace('.', '', $request['precio_unitario'][$i]) );
            ...
            {
                $gravado_excenta += $precio_unitario*$cantidad*$cambio_int;
            }
        }

        $gravado_excenta = intval($gravado_excenta);
        $gravado_5 = intval($gravado_5);
        $gravado_10 = intval($gravado_10);
        $iva_10 = intval($iva_10);
        $iva_5 = intval($iva_5);

        $gravado_10 = $total_item_con_iva_10 - $iva_10;

        $gravado_5 = $total_item_con_iva_5 - $iva_5;

        $total_suma = $gravado_excenta + $gravado_5 + $gravado_10 + $iva_10 + $iva_5;


        //dd('EXCENTA='.$gravado_excenta.' GRAVADO 5% ='.$gravado_5.' GRAVADO 10% ='.$gravado_10.' IVA 5%='.$total_iva_5.' IVA 10%='.$total_iva_10.' TOTAL='.$request->monto_total.' TOTAL DE SUMA='.$total_suma);
        ...
        try
        {
            ...
            for($i = 0; $i < count($cantidad); $i++)
            {
                ...
                if($request->get('isProducto') == '1')
                {
                    ...
                    if ($stock_producto + $cantidad[$i] != 0){
                      $nuevo_precio_compra_promedio = number_format( (($stock_producto*$precio_compra_producto)+($cantidad[$i]*str_replace('.', '', $precio_unitario[$i])) )/($stock_producto+$cantidad[$i]), 3, '.', '');
                    }else{
                      $nuevo_precio_compra_promedio = number_format( str_replace('.', '', $precio_unitario[$i] ), 3, '.', '');
                    }
                ...
                foreach ($detalles as $key => $detalle)
                {
                    if($detalle->producto_id)
                    {
                        ...
                        if( $stock_menos_cantidad_comprada == 0 )
                            $old_precio_compra_promedio = $detalle->precio_unitario;
                        else
...
/*---------------------------details------------------------------------------*/
    ...
    {
        $opciones_categoria = ['Producto', 'Otro', 'Salario'];
    ...
    public function query_reporte(Request $request)
    {
        ...
        if ($categoria != 'Salario')
        {
            ...
            ->select('compras__compras.*','proveedores__proveedors.razon_social as razon_social')
        ...
        else
        {
            ...
            $object = Datatables::of( $query )
                ...
                ->editColumn('total', function($tabla) use ($query)
                {
                    ...
                    $monto_total = $tabla->extra + $tabla->monto_ips+ $tabla->salario;
